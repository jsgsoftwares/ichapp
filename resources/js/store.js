...
export default new Vuex.Store({
    ...
    mutations: {
        setplansubscrito(state, plans) {
            state.plansubscrito = plans;
        },
        setusersubscritos(state, usuarios) {

            state.usuariossubscritos = usuarios;
        },
        setproductsplanes(state, products) {
            state.productsubscriptos = products;
        },
        ...
        setsubscripcion(state, subscripto) {
            state.subscripcion = subscripto;
        },
        setIntegraciones(state, integra) {
            state.integraciones = integra;
        },
        setlistplanes(state, plan) {
            state.planes = plan;
        },
    ...
    actions: {
        //PUT
        Putcancelplansubscrito(context, id_subs) {
            const params = { id: id_subs }
            console.log(params)
            return axios
                .post(`/api/v1/cancelar/subscription`, params)
                .then((response) => {
                    console.log("cancelados", response.data);
        ...
        getUserCompanie_dash(context, userId) {
            return axios.get(`/get/usuarios/companie/${userId}`).then((response) => {
                // console.log("response usuarios axti", response.data);
        ...
        getPlanes(context) {
            return axios.get("/api/v1/get/planes/subscripcion").then((response) => {
                //console.log("response", response.data);
                context.commit("setlistplanes", response.data);
            });
        },
        ...
        getSubscripciones(context, usuario) {
            // const parametros = { user_id: usuario };

            return axios
                .get(`/api/v1/get/subscription/${context.state.user_id}`)
                .then((response) => {
                    // console.log('integracion__', response.data)
                    context.commit("setsubscripcion", response.data);
                })
                .catch((error) => {
        ...
        Postsubscription(context, parametros) {
            console.l('en store', parametros);
        },
        //INTEGRACIONES
        Postproductsplansubscrito(context) {
            const params = { user_id: context.state.user_id }
            return axios
                .post(`/api/v1/get/subscrito/plan`, params)
                .then((response) => {

                    context.commit("setplansubscrito", response.data);
                })
                .catch((error) => {
                    console.log("error", error);
                });
        ...
        Postproductsplanes(context) {
            const params = { user_id: context.state.user_id }
            return axios
                .post(`/api/v1/get/subscripcion/plan`, params)
                .then((response) => {

                    context.commit("setproductsplanes", response.data);
                })
                .catch((error) => {
                    console.log("error", error);
                });
        ...
        PostUsuariosSubscritos(context) {
            const parametros = { user_id: context.state.user_id };

            return axios
                .post("/api/v1/obtener/usuarios/subscritos", parametros)
                .then((response) => {

                    context.commit("setusersubscritos", response.data);
                })
                .catch((error) => {
                    console.log(error.response);
                });
        },
        PostUsuarioDisable(context, parametros) {


            return axios
                .post("/api/v1/disabled/usuario", parametros)
                .then((response) => {
        ...
        PostUsuariosEnabled(context, parametros) {


            return axios
                .post("/api/v1/enabled/usuario", parametros)
                .then((response) => {
